jQuery(document).ready(function() {
	...
	$(document.body).on('click', '.que_view .unpublish_subject' ,function(){
...
//		alert("Hello edit " + sid);
	...
	$(document.body).on('click', '.que_view .publish_subject' ,function(){
		var sid = $(this).parent().parent().attr("sid");
	...
	$(document.body).on('click', '.que_view .delete_subject' ,function(){
		var sid = $(this).parent().parent().attr("sid");
...
function getQuestionList(pn)
{
//	alert("get List")
	$.ajax({
		type : "GET",
		url : "getAdminQuestionList",
		data : {'pn':pn},
		contentType : "application/json",
		success : function(data) {
			$(".bite_que .que_view").html(data);
		},
		error: function (xhr, ajaxOptions, thrownError) {
	        alert(xhr.status);
	      }
